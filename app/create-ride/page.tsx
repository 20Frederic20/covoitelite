      ...
      <div className="pb-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
            ...
            <div className="relative">
              <label className="text-xs text-muted-foreground uppercase font-bold mb-1 block">Destination</label>
              <div className="flex items-center gap-3">
                <MapPin size={20} className="text-primary" />
                <input
                  type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-card border border-border rounded-2xl p-4">
              <label className="text-xs text-muted-foreground uppercase font-bold mb-2 block">Date</label>
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="bg-transparent border-none focus:ring-0 text-sm text-foreground w-full p-0"
                />
              </div>
            </div>
            <div className="bg-card border border-border rounded-2xl p-4">
              <label className="text-xs text-muted-foreground uppercase font-bold mb-2 block">Heure</label>
              <div className="flex items-center gap-2">
          ...
          <div className="bg-card border border-border rounded-2xl p-6 space-y-6">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-foreground">Prix par place</span>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="number"
                  ...
                  className="bg-muted border-none rounded-lg w-20 text-right p-2 text-primary font-bold"
                />
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-foreground">Places disponibles</span>
              </div>
              <div className="flex items-center gap-4">
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, seats: Math.max(1, parseInt(formData.seats) - 1).toString() })}
                  className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-xl text-foreground"
                >
                  -
                </button>
                <span className="font-bold text-lg text-foreground">{formData.seats}</span>
                <button
                  type="button"
            ...
            <div className="flex justify-between items-center">
              ...
              <input
                ...
                className="bg-muted border-none rounded-lg w-32 p-2 text-sm text-foreground"
              />
            </div>
          </div>
          ...
          <div className="bg-muted/50 p-4 rounded-xl border border-border/50">
            <div className="flex justify-between text-sm mb-1">
              <span className="text-muted-foreground">Commission CovoitElite (10%)</span>
              <span className="text-muted-foreground">-{formData.price ? (parseInt(formData.price) * 0.1).toFixed(0) : 0} FCFA</span>
            </div>
        ...
        {isSuccess && (
          ...
          >
            <div className="bg-card border border-border rounded-3xl p-8 text-center max-w-xs w-full">
              ...
              <h2 className="text-2xl font-bold mb-2 text-foreground">Trajet publié !</h2>
              <p className="text-muted-foreground">Votre trajet est maintenant visible par tous les passagers.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
