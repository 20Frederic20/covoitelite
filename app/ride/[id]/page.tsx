      ...
      <div className="pb-20">
        ...
        {user?.id === ride.driverId && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 mb-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-sm font-bold text-zinc-500 uppercase tracking-widest">Gestion du trajet</h3>
              <button 
                onClick={() => setShowBookings(!showBookings)}
                className="bg-primary text-black px-4 py-2 rounded-xl text-xs font-bold hover:bg-yellow-500 transition-colors"
              >
                  ...
                  {rideBookings.length > 0 ? (
                    <div className="grid gap-4">
                      {rideBookings.map((booking) => (
                        <div key={booking.id} className="bg-black/40 border border-zinc-800 rounded-2xl p-4 flex justify-between items-center">
                          <div>
                            <p className="font-bold text-sm">{booking.passengerName}</p>
                            <div className="flex items-center gap-2 text-[10px] text-zinc-500 uppercase font-bold mt-1">
                              <Users size={10} />
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-[10px] text-zinc-500 uppercase font-bold">Total payé</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-center py-8 bg-black/20 rounded-2xl border border-dashed border-zinc-800">
                      <p className="text-zinc-500 text-sm italic">Aucune réservation pour le moment.</p>
                    </div>
                  )}
                </motion.div>
              )}
